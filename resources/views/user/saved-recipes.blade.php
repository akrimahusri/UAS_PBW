...
<div class="w-full bg-white flex flex-col min-h-screen">
    <div class="px-6 md:px-10 py-3 border-b border-gray-200 flex justify-end items-center">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Saved Recipes</h1>
    </div>
    ...
    <div class="flex-grow p-6 md:p-10 flex flex-col">
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col">
                    ...
                    <div class="p-3 flex justify-end space-x-2" style="background-color: #AED5D6; border-top: 1px solid rgba(255,255,255,0.5);">
                        ...
                        <form action="{{ route('recipes.unsave', $recipe->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-xs bg-blue-500 text-white py-1.5 px-3 rounded-md hover:bg-blue-600 transition-colors">Unsave</button>
                        </form>
                    </div>
                </div>
                @endforeach
        ...
        @else
            <div class="flex-grow flex flex-col items-center justify-center text-center mt-6">
                ...
                <p class="text-xl text-gray-400">No saved recipes yet.</p>
                <p class="mt-2 text-sm text-gray-500">
                    Explore the <a href="{{ route('dashboard') }}" class="text-[#3DA0A7] hover:underline font-medium">dashboard</a> to find recipes to save!
                </p>
            </div>
        @endif
    </div>
